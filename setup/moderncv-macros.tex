...
\newcommand{\interest}[2]{
    \begin{minipage}{0.95\linewidth} % Reduced width to add whitespace on the right
...
\newcommand{\work}[4]{
    ...
    \noindent \large \color{highlight} \parbox{.7\linewidth}{\textsl{\texthead{#3}}} \par
...
\newcommand{\publication}[5]{
    % Title
    \vspace{-0.4em}
    \noindent \large \parbox{.8\linewidth}{\texthead{#1} \link{#5}}
    % Publication Date
    \hfill \small
    \tcbox[enhanced,box align=base,nobeforeafter,colback=title,colframe=title,size=fbox,arc=0mm]{\texthead{#2}} \par
    \vspace{0.5em}
